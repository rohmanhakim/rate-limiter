...
func TestMaxDuration(t *testing.T) {
	t.Run("returns zero for empty slice", func(t *testing.T) {
		result := MaxDuration([]time.Duration{})
		if result != 0 {
	...
	t.Run("returns single element", func(t *testing.T) {
		result := MaxDuration([]time.Duration{5 * time.Second})
		if result != 5*time.Second {
			t.Errorf("expected 5s, got %v", result)
		}
	})

	t.Run("returns max from multiple elements", func(t *testing.T) {
		result := MaxDuration([]time.Duration{
			1 * time.Second,
			5 * time.Second,
			3 * time.Second,
	...
	t.Run("handles all zeros", func(t *testing.T) {
		result := MaxDuration([]time.Duration{0, 0, 0})
		if result != 0 {
			t.Errorf("expected 0, got %v", result)
		}
	})
	...
	t.Run("does not mutate input slice", func(t *testing.T) {
		input := []time.Duration{3 * time.Second, 1 * time.Second, 2 * time.Second}
		MaxDuration(input)
		// Original order should be preserved
		if input[0] != 3*time.Second || input[1] != 1*time.Second || input[2] != 2*time.Second {
